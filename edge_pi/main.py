...
def report_intake(
    ...
    *,
    ...
    HI-012: ``is_stub`` round-trips into the queue row so the replay loop
    can refuse to post falsified telemetry.
    Phase 9: ``confidence`` (when not None) is the maximum YOLO
    observation seen during ``PillVerifier.confirm_tray_empty`` and is
    sent as ``confidence_score`` (Phase 1 column on adherence_logs).
    Stub-mode and pre-Phase-9 callers omit the kwarg → no field in the
    body → backend stores NULL (the [0,1] CHECK allows NULL).
    """
    assert session is not None, "session not initialized; call run() first"
