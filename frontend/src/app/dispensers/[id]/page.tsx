...
export default function DispenserGuidedPage() {
  ...
  // Cognitive-science honesty assessment shown at the Log step (after the
  // success modal). Generated once per terminal intake round.
  ...
  // Generate the cognitive-science honesty assessment once the intake round
  // reaches a terminal result. Keyed by started_at so it fires once per round
  // and re-runs for the next round. Survives the 250 ms intake poll.
  useEffect(() => {
    const result = intake?.result ?? null;
    if (!intake || result === null) return;
    const roundKey = intake.started_at ?? null;
    if (roundKey !== null && analysisRoundRef.current === roundKey) return;
    analysisRoundRef.current = roundKey;
    ...
    void analyzeIntake({
      ...
      hold_progress: intake.hold_progress,
      mediapipe_complete: intake.mediapipe_complete,
      labels_seen: intake.labels_seen,
      labels_required: intake.labels_required,
      labels_satisfied: intake.labels_satisfied,
      history: intake.history,
      started_at: intake.started_at,
      ended_at: intake.ended_at,
      patient_name: activePatient?.name ?? null,
      medication: currentSlot?.name ?? null,
      slot: currentSlot?.slot ?? null,
    })
      .then((a) => {
  ...
  // Re-arm the honesty assessment when a fresh watch starts, so a stale report
  // from the previous round isn't shown while the next one is in progress.
  useEffect(() => {
    if (intake?.running && intake.result === null) {
      analysisRoundRef.current = null;
      setAnalysis(null);
    }
  }, [intake?.running, intake?.result]);
...
// ─────────────────────── CognitiveAnalysisCard ──────────────────────────────
// LLM honesty assessment shown to the nurse/pharmacist at the Log step, after
// the success modal. Frames the swallow-FSM + vision-label signals through a
// cognitive-science lens (multi-channel corroboration) to flag a possibly
// feigned/concealed intake. Generated by lib/agent.analyzeIntake.

const RISK_STYLE: Record<
  DeceptionRisk,
  { label: string; chip: string; border: string; bg: string; icon: string }
> = {
  low: {
    label: "Low risk · corroborated",
  ...
  high: {
    label: "High concern · possible feigned intake",
    chip: "bg-status-danger-bg text-status-danger",
    border: "border-status-danger",
    bg: "bg-status-danger-bg/40",
    icon: "⚠",
  },
};

function CognitiveAnalysisCard({
  analysis,
  loading,
}: {
  ...
  const risk = analysis?.deception_risk ?? "moderate";
  const style = RISK_STYLE[risk];

  return (
    <div
      className={`mt-4 rounded-2xl border ${
        analysis ? style.border : "border-sand-200"
      } bg-white p-5`}
    >
      <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
        <div className="flex items-center gap-2">
          <span className="text-[11px] font-medium uppercase tracking-wider text-gray-400">
            Cognitive analysis · intake honesty
          </span>
          <span
            className="rounded-full bg-olive-100 px-2 py-0.5 text-[10px] font-semibold text-olive-700"
            title="Decision support — not a verdict of guilt"
          >
            AI · decision support
          </span>
        </div>
      ...
      {loading && !analysis ? (
        <div className="flex items-center gap-3 py-4 text-sm text-gray-500">
          <span className="h-3 w-3 animate-spin rounded-full border-2 border-olive-300 border-t-olive-700" />
          Analysing intake behaviour across motor, object, and honesty channels…
        </div>
      ) : analysis ? (
        <>
          <div
            className={`rounded-xl border ${style.border} ${style.bg} p-4 text-sm leading-relaxed text-gray-800 [&_h2]:mb-2 [&_h2]:text-base [&_h2]:font-semibold [&_h2]:text-gray-900 [&_h3]:mb-1 [&_h3]:mt-3 [&_h3]:text-[11px] [&_h3]:font-semibold [&_h3]:uppercase [&_h3]:tracking-wider [&_h3]:text-gray-500 [&_li]:ml-4 [&_li]:list-disc [&_p]:mb-2 [&_ul]:mb-2 [&_ul]:space-y-1`}
          >
            <ReactMarkdown>{analysis.content_markdown}</ReactMarkdown>
          </div>
          <p className="mt-3 text-[10px] text-gray-400">
            {analysis.metadata.used_llm
              ? `Generated by ${analysis.metadata.model} · ${analysis.metadata.latency_ms} ms`
              : "Deterministic on-device assessment (LLM narrator offline)."}{" "}
            · Cognitive-science framing (multi-channel corroboration). Supports,
            does not replace, a direct mouth check.
          </p>
        </>
      ) : null}
    </div>
